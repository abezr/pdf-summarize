...
const graph_builder_1 = require("../../src/services/graph/graph-builder");
const reference_validation_service_1 = require("../../src/services/graph/reference-validation.service");
const reference_accuracy_tester_1 = require("../../src/services/graph/reference-accuracy-tester");
describe('Reference Detection Integration', () => {
    // Create mock PDF result with references
    const createMockPDFResult = () => ({
        metadata: {
            title: 'Test Document',
            pages: 5,
            fileSize: 100000,
        ...
        pages: [
            {
                ...
                width: 612,
                height: 792,
                content: 'This is the introduction. See section 2.1 for methodology details.',
                textElements: [],
                paragraphs: [
                    {
                        content: 'This is the introduction. See section 2.1 for methodology details.',
            ...
            },
            {
                ...
                width: 612,
                height: 792,
                content: 'Methodology Section. As shown in Figure 1, the approach works well.',
                textElements: [],
                paragraphs: [
                    {
                        content: 'Methodology Section. As shown in Figure 1, the approach works well.',
            ...
            },
            {
                ...
                width: 612,
                height: 792,
                content: 'Results are presented in Table 1. Check page 4 for more details.',
                textElements: [],
                paragraphs: [
                    {
                        content: 'Results are presented in Table 1. Check page 4 for more details.',
    ...
    describe('Complete Pipeline Integration', () => {
        test('should build graph with reference detection', async () => {
            ...
            const graph = await graph_builder_1.GraphBuilder.buildGraph('test-doc', pdfResult);
        ...
        test('should validate reference detection results', async () => {
            const pdfResult = createMockPDFResult();
            const graph = await graph_builder_1.GraphBuilder.buildGraph('test-doc', pdfResult);
            const validation = await reference_validation_service_1.ReferenceValidationService.validateGraph(graph);
        ...
        test('should detect expected reference types in graph', async () => {
            ...
            const edgeContexts = referenceEdges
                .map((e) => e.metadata?.context)
                .filter(Boolean);
            const hasSectionRef = edgeContexts.some((ctx) => ctx.includes('section'));
            const hasFigureRef = edgeContexts.some((ctx) => ctx.includes('Figure'));
            const hasTableRef = edgeContexts.some((ctx) => ctx.includes('Table'));
            const hasPageRef = edgeContexts.some((ctx) => ctx.includes('page'));
            expect(hasSectionRef).toBe(true);
            expect(hasFigureRef).toBe(true);
    ...
    describe('Accuracy Testing Integration', () => {
        test('should run accuracy test suite', async () => {
            const suite = reference_accuracy_tester_1.ReferenceAccuracyTester.createStandardTestSuite();
            expect(suite).toBeDefined();
            expect(suite.testCases.length).toBeGreaterThan(0);
            // Run a single test case to verify the framework works
            const testResult = await reference_accuracy_tester_1.ReferenceAccuracyTester.runTestCase(suite.testCases[0]);
        ...
        test('should calculate detection accuracy metrics', async () => {
            ...
            const result = await reference_accuracy_tester_1.ReferenceAccuracyTester.runTestCase(testCase);
        ...
        test('should identify detection issues', async () => {
            ...
            expect(result.detectionResults.falseNegatives).toBeGreaterThan(0);
        });
    });
    ...
    describe('Performance Validation', () => {
        ...
        test('should handle increasing document complexity', async () => {
            ...
            const smallGraph = await graph_builder_1.GraphBuilder.buildGraph('small-doc', smallDoc);
            ...
            const largeGraph = await graph_builder_1.GraphBuilder.buildGraph('large-doc', largeDoc);
