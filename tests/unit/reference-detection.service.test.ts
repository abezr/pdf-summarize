...
describe('Reference Detection Service', () => {
  let mockParagraphNode: GraphNode;
  let mockSectionNode: GraphNode;
  let mockImageNode: GraphNode;
  ...
  test('should analyze paragraph nodes for references', async () => {
    const analysis =
      await ReferenceDetectionService.analyzeNode(mockParagraphNode);

    expect(analysis.sourceNode).toBe(mockParagraphNode);
    expect(analysis.references.length).toBeGreaterThan(0);
    expect(analysis.metadata.textLength).toBe(
      mockParagraphNode.content.length
    );
  ...
  test('should analyze section nodes for references', async () => {
    const analysis =
      await ReferenceDetectionService.analyzeNode(mockSectionNode);
  ...
  test('should reject non-text nodes', async () => {
    await expect(
      ReferenceDetectionService.analyzeNode(mockImageNode)
    ).rejects.toThrow('Cannot analyze references in non-text node');
  });
  ...
  test('should calculate accurate statistics', async () => {
    ...
    expect(analysis.stats.confidence.average).toBeGreaterThanOrEqual(0);
    expect(analysis.stats.confidence.average).toBeLessThanOrEqual(1);
    expect(analysis.stats.confidence.min).toBeGreaterThanOrEqual(0);
    expect(analysis.stats.confidence.min).toBeLessThanOrEqual(1);
    expect(analysis.stats.confidence.max).toBeGreaterThanOrEqual(0);
    expect(analysis.stats.confidence.max).toBeLessThanOrEqual(1);
  });

  describe('Text Analysis', () => {
    test('should analyze plain text for references', async () => {
      const text = 'See section 2.1 and Figure 3 for details.';
      const analysis = await ReferenceDetectionService.analyzeText(text);
  ...
  describe('Multiple Node Analysis', () => {
    test('should analyze multiple nodes and skip non-text ones', async () => {
      ...
      const analyses = await ReferenceDetectionService.analyzeNodes(nodes);
    ...
    test('should handle empty node array', async () => {
      const analyses = await ReferenceDetectionService.analyzeNodes([]);
  ...
  describe('Reference Filtering', () => {
    test('should filter references by target type', async () => {
      ...
      const sectionRefs =
        ReferenceDetectionService.filterReferencesByTargetType(
          analysis.references,
          'section'
        );
      const figureRefs = ReferenceDetectionService.filterReferencesByTargetType(
        analysis.references,
        'image'
      );
      ...
      expect(
        sectionRefs.every(
          (ref) => ref.type === 'section' || ref.type === 'cross_reference'
        )
      ).toBe(true);
  ...
  describe('Validation', () => {
    test('should validate analysis results', async () => {
      const testParagraphNode: GraphNode = {
        id: 'test-paragraph',
        type: 'paragraph',
        label: 'Test Paragraph',
        content: 'See section 3.2 for details.',
        position: { page: 1, start: 0, end: 30 },
        created_at: new Date(),
        updated_at: new Date(),
        metadata: {
          confidence: 0.9,
      ...
      const analysis = await ReferenceDetectionService.analyzeNode(testParagraphNode);
      const validation = ReferenceDetectionService.validateAnalysis(analysis);
    ...
    test('should identify issues with empty text', async () => {
      const emptyNode: GraphNode = {
        ...
        label: 'Empty',
        content: '',
        position: { page: 1, start: 0, end: 0 },
        created_at: new Date(),
        updated_at: new Date()
      };
      const analysis = await ReferenceDetectionService.analyzeNode(emptyNode);
      const validation = ReferenceDetectionService.validateAnalysis(analysis);
    ...
    test('should calculate summary statistics', () => {
      const analyses = [
        {
          ...
          references: [],
          referencesByType: {} as Record<ReferenceType, DetectedReference[]>,
          stats: {
            totalReferences: 5,
            uniqueTargets: 3,
            confidence: { average: 0.8, min: 0.6, max: 0.9 },
            types: {} as Record<ReferenceType, number>,
          },
          metadata: {
      ...
      const summary = ReferenceDetectionService.getSummaryStats(analyses);
  ...
  describe('Node Type Detection', () => {
    test('should identify text nodes correctly', () => {
      ...
      expect(ReferenceDetectionService['isTextNode'](paragraphNode)).toBe(true);
      expect(ReferenceDetectionService['isTextNode'](sectionNode)).toBe(true);
      expect(ReferenceDetectionService['isTextNode'](imageNode)).toBe(false);
    });
  });
  ...
  describe('Text Extraction', () => {
    test('should extract text from different node types', () => {
      ...
      expect(
        ReferenceDetectionService['extractTextForAnalysis'](paragraphNode)
      ).toBe('Simple paragraph text');
      expect(
        ReferenceDetectionService['extractTextForAnalysis'](metadataNode)
      ).toBe('extracted from properties');
    });
  });
});