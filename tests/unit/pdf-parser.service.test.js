...
const pdf_parser_service_1 = require("../../src/services/pdf-parser.service");
...
describe('PDFParserService', () => {
    let pdfParser;
    beforeEach(() => {
        pdfParser = new pdf_parser_service_1.PDFParserService();
    });
    describe('isValidPDF', () => {
        it('should return true for valid PDF buffer', () => {
            const result = pdfParser.isValidPDF(mockData_1.mockFileBuffers.smallPDF);
    ...
    describe('parsePDF', () => {
        it('should parse a valid PDF and return structured data', async () => {
            const result = await pdfParser.parsePDF(mockData_1.mockFileBuffers.smallPDF, 'test.pdf');
            ...
            expect(result.metadata.fileSize).toBe(mockData_1.mockFileBuffers.smallPDF.length);
        ...
        it('should handle PDF with metadata', async () => {
            ...
            const result = await pdfParser.parsePDF(mockData_1.mockFileBuffers.smallPDF);
            expect(result.metadata).toBeDefined();
        ...
        it('should throw AppError for invalid PDF', async () => {
            ...
            await expect(pdfParser.parsePDF(invalidBuffer)).rejects.toThrow(errors_1.AppError);
        });
    ...
    describe('extractText', () => {
        it('should extract text content from PDF', async () => {
            const text = await pdfParser.extractText(mockData_1.mockFileBuffers.smallPDF);
    ...
    describe('extractParagraphs', () => {
        it('should extract paragraphs with metadata from PDF', async () => {
            const paragraphs = await pdfParser.extractParagraphs(mockData_1.mockFileBuffers.smallPDF);
    ...
    describe('getParagraphStats', () => {
        it('should calculate paragraph statistics', async () => {
            const stats = await pdfParser.getParagraphStats(mockData_1.mockFileBuffers.smallPDF);
    ...
    describe('extractMetadata', () => {
        it('should extract metadata without full text parsing', async () => {
            const metadata = await pdfParser.extractMetadata(mockData_1.mockFileBuffers.smallPDF, 'test.pdf');
            expect(metadata).toBeDefined();
            expect(metadata.title).toBe('Sample Document');
            expect(metadata.author).toBe('Test Author');
            expect(metadata.pages).toBe(1);
            expect(metadata.fileSize).toBe(mockData_1.mockFileBuffers.smallPDF.length);
        ...
        it('should handle invalid PDF for metadata extraction', async () => {
            // Create a buffer larger than 100 bytes but with invalid PDF content
            const invalidBuffer = Buffer.from('not a pdf file at all, this is just some text that is definitely not a PDF document and should be rejected');
            await expect(pdfParser.extractMetadata(invalidBuffer)).rejects.toThrow(errors_1.AppError);
        });
