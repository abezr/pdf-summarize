...
const reference_matcher_1 = require("../../src/services/graph/reference-matcher");
describe('Reference Matcher', () => {
    describe('Basic Functionality', () => {
        test('should detect references in text', () => {
            ...
            const result = reference_matcher_1.ReferenceMatcher.findReferences(text);
        ...
        test('should detect multiple reference types', () => {
            const text = 'See section 2.1, Figure 3, and Table 4. Check page 15 for details.';
            const result = reference_matcher_1.ReferenceMatcher.findReferences(text);
        ...
        test('should include context around references', () => {
            const text = 'According to the methodology described in section 2.1, the results show clear trends.';
            const result = reference_matcher_1.ReferenceMatcher.findReferences(text, 30);
            for (const ref of result.references) {
    ...
    describe('Reference Type Filtering', () => {
        test('should filter by section references only', () => {
            ...
            const sections = reference_matcher_1.ReferenceMatcher.findReferencesByType(text, 'section');
            expect(sections.every((ref) => ref.type === 'section')).toBe(true);
            expect(sections.length).toBeGreaterThan(0);
        });
        test('should filter by figure references only', () => {
            const text = 'See section 3.2, Figure 1, and Table 4.';
            const figures = reference_matcher_1.ReferenceMatcher.findReferencesByType(text, 'figure');
    ...
    describe('Validation', () => {
        test('should validate results without issues for good matches', () => {
            const text = 'See section 3.2 for details.';
            const result = reference_matcher_1.ReferenceMatcher.findReferences(text);
            const validation = reference_matcher_1.ReferenceMatcher.validateResults(result);
        ...
        test('should identify issues with excessive matches', () => {
            ...
            const validation = reference_matcher_1.ReferenceMatcher.validateResults(mockResult);
    ...
    describe('Edge Cases', () => {
        test('should handle empty text', () => {
            const result = reference_matcher_1.ReferenceMatcher.findReferences('');
        ...
        test('should handle very long text', () => {
            const longText = 'Section 1. '.repeat(1000) + 'See section 3.2 for details.';
            const result = reference_matcher_1.ReferenceMatcher.findReferences(longText);
            expect(result.references.length).toBeGreaterThan(0);
