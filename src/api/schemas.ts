...
export const CreateDocumentSchema = z.object({
  user_id: z.string().optional(),
  filename: z.string().min(1).max(255).regex(/\.(pdf|PDF)$/, 'Filename must end with .pdf'),
...
export const DocumentQuerySchema = z.object({
  ...
  order_by: z.enum(['created_at', 'updated_at', 'filename']).default('created_at'),
...
export const SummarizationOptionsSchema = z.object({
  type: z.enum(['executive', 'detailed', 'chapter', 'bullet-points', 'narrative', 'technical']).default('executive'),
...
export const FileUploadSchema = z.object({
  file: z.any().refine((file) => {
    ...
    return file && typeof file === 'object' &&
           file.originalname && file.mimetype && file.size !== undefined;
  }, 'Valid file is required').refine((file) => {
    // Check file type
...
export const SummarizationResponseSchema = z.object({
  ...
  data: z.object({
    summary: z.string(),
    type: z.enum(['executive', 'detailed', 'chapter', 'bullet-points', 'narrative', 'technical']),
    model: z.string(),
    provider: z.string(),
